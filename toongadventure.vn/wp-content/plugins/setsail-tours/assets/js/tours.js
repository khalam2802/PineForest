(function ($) {
  ...
  function destinationShortcodeSearchFilters() {
    var $searchForms = $(
      ".qodef-tours-filter-holder.qodef-tours-filter-horizontal form"
    );
    var $searchFormsVerticalSmall = $(
      ".qodef-tours-filter-holder.qodef-tours-filter-vertical-small form"
    );

    var fieldsHelper = (function () {
      var initDestinationSearch = function () {
        var destinations =
          typeof qodefToursSearchData !== "undefined"
            ? qodefToursSearchData.destinations
            : [];
        ...
        $searchFormsVerticalSmall
          .find(".qodef-tours-destination-search")
          .typeahead(
            {
              hint: true,
              highlight: true,
...
(function ($) {
  ...
  function qodefTourPriceSingleTemplate() {
    ...
    if (price_el.length !== 0) {
      var data = {
        action: "dep365_get_price_single_tour_ajax",
        id_tour: id_tour,
      };
      $.ajax({
  ...
  function qodefTrigerTourGalleryMasonry() {
    ...
    tourNavItems.on("click", function () {
      ...
      if (
        thisNavItemId === "#tour-item-gallery-id" &&
        typeof qodef.modules.common.qodefInitGridMasonryListLayout ===
          "function"
      ) {
  ...
  function searchTours() {
        ...
        if ($rangeSliders.length) {
          $rangeSliders.each(function () {
            ...
            slider.on("update", function (values) {
              ...
              var currencySymbolPosition = $priceRange.data(
                "currency-symbol-position"
              );

              var firstPrice =
                currencySymbolPosition === "left"
                  ? currencySymbol + firstValue
                  : firstValue + currencySymbol;
              var secondPrice =
                currencySymbolPosition === "left"
                  ? currencySymbol + secondValue
    ...
    var handleSearch = (function () {
      ...
      var sendRequest = function (
        ...
        resetPagination =
          typeof resetPagination !== "undefined" ? resetPagination : true;
        ...
        if (!searchInProgress) {
          ...
          $.ajax({
            ...
            success: function (response) {
              ...
              $searchContent
                .find(".qodef-tours-row .qodef-tours-row-inner-holder")
                .html(response.html);
              ...
              if (animate) {
                $("html, body").animate(
                  { scrollTop: $searchPageContent.offset().top - 80 },
                  700
                );
              }
              qodefToursGalleryAnimation();
      ...
      var handlePagination = function ($searchForms) {
        ...
        if ($paginationHolder.length) {
          $(document).on(
            "click",
            ".qodef-tours-search-pagination li",
            function (e) {
              e.preventDefault();
              e.stopPropagation();
    ...
    if (toursGalleryItems.length) {
      toursGalleryItems.each(function () {
        var toursGalleryItem = $(this),
          contentHolder = toursGalleryItem.find(
            ".qodef-tours-gallery-item-content-holder"
          ),
...
(function ($) {
  ...
  function qodefOnWindowLoad() {
    toursList().init();
    let $qodef = $(document).find(
      '.qodef-tour-list-filter-item[data-type="tour-category-mien-nam"]'
    );
    console.log($qodef);
    $qodef.find("a").click();
  }
  ...
  function toursList() {
    ...
    var filter_tour = function (filters, listFeed) {
      ...
      filters
        .parent()
        .find(".qodef-tour-list-filter-item")
        .each(function (e) {
    ...
    var initPagination = function (listObject) {
      var paginationDataHolder = listObject.find(
        ".qodef-tours-list-pagination-data"
      );
        ...
        $.ajax({
          ...
          success: function (response) {
            if (response.havePosts) {
              paginationDataHolder
                .find('[name="next_page"]')
                .val(response.nextPage);
            }
      ...
      var loadMorePagination = function () {
        ...
        var paginationHolder = listObject.find(
          ".qodef-tours-pagination-holder"
        );
              ...
              fetchData(function (response) {
                if (response.havePosts === true) {
                  ...
                  itemsHolder.waitForImages(function () {
                    ...
                    filter_tour(
                      listObject.find(".myauris_temp_class"),
                      listObject
                    );
    ...
    var resizeTourItems = function resizeTourItems(container) {
      ...
      if (itemsMainHolder.hasClass("qodef-tours-type-masonry")) {
        var padding = parseInt(
            container.find(".qodef-tours-row-item").css("padding-left")
          ),
          ...
          largeWidthHeightMasonryItem = container.find(
            ".qodef-size-large-width-height"
          ),
          size = container.find(".qodef-tours-list-grid-sizer").width();

        defaultMasonryItem.css("height", size - 2 * padding);
        largeHeightMasonryItem.css(
          "height",
          Math.round(2 * size) - 2 * padding
        );
        largeWidthHeightMasonryItem.css(
          "height",
          Math.round(2 * size) - 2 * padding
        );
